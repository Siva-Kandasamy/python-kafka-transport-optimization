...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == 'org.cta.stations.faust.tbl1': # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif f"com.cta.station.arrival_dtl.tbl1" in message.topic(): # Set the conditional to the arrival topic
